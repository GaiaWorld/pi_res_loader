...
impl LoadMgr {
	...
	/// 取到资源
	pub fn get<D, A: Asset<D>>(&mut self, key: &A::Key, group: usize) -> Option<Share<A>> {
		self.res_mgr.get(key, group)
	}

	/// 创建资源
	/// 如果get方法取不到资源，调用该方法创建一个资源
	pub fn create<D, A: Asset<D>>(&mut self, key: A::Key, group_i: usize, cost: usize, asset: A, desc: D) -> BoxFuture<'static, ()> {
		let res = self.res_mgr.create(key, group_i, asset, cost);
		self.load(res, desc)
	}

	// 加载资源
	pub fn load<D, A: Asset<D>>(&mut self, asset: Share<A>, desc: D) -> BoxFuture<'static, ()> {
		match asset.state() {
